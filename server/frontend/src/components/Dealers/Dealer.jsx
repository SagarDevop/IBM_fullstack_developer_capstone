...
const Dealer = () => {
  ...
  const get_dealer = async () => {
    try {
      ...
      if (!res.ok) throw new Error("Network error fetching dealer");
  ...
  const get_reviews = async () => {
    try {
      ...
      if (!res.ok) throw new Error("Network error fetching reviews");
      const retobj = await res.json();
    ...
    <div style={{ margin: "20px" }}>
      ...
      <div style={{ marginTop: "10px" }}>
        {dealer && dealer.full_name ? (
          ...
          <h3 style={{ color: "grey" }}>Loading dealer details...</h3>
        )}
      </div>
